...
The setup is similar to the one before, except that now we need to model the difference `DURING - BEFORE` for each `Category` level. 
...
\epsilon_i
$$
In this case the terms $b_{l, D}(t_i)$ and $b_{h, D}(t_i)$ represent the difference `DURING - BEFORE` for `low` and `high` respectively. We are using the convention whereby `BEFORE` is the default level, so the other binsry wmooths represent the differences  `AFTER - BEFORE`, which we do not need for H2. 
...
**Important note: ** some of the binary smooths in this model are defined by the same interaction conditions from the previous model. However, their value is **not** the same, as it is the **combination** of ordinary and binary smooths that defines what they actually model. 

### H3: `DURING`: `high` curves higher than `low` curves

We are lucky, as this hypothesis can be tested by reusing the model we used to test H1. We are going to check and plot $b_{D, h}(t_i)$. From the `summary` above we see that the term is significant. H3 requires is to be above the zero line, as it represents the difference `high - low` for the `DURING` `State`. 
...
$$ (y\big\rvert{h, B} - y\big\rvert{h, D}) -  (y\big\rvert{l, D} - y\big\rvert{l, B}) > 0$$
where $y\big\rvert{h, B}$ means "the predicted $y$ at  `Category` `high` and `State` `BEFORE`", and similarly for the other terms. 
...
There are two reasons why this is not enough in our case. First, with just one set of $b$ terms we will be able to express differences between levels, but not differences of differences. So we need two sets of $b$ terms with different meanings. In particular, we need the term $y\big\rvert{h, B} - y\big\rvert{h, D}$ to contain one $b$ element more than the term $y\big\rvert{l, D} - y\big\rvert{l, B}$, so that everything cancels out and only one $b$ term remains. 
Second, there is no way to create the asymmetry in the subtractions that we need by using $f$ terms were based on either `Category` or `State`.
...
This new factor `X` behaves similarly to `State` in that it divides the 6 cells into 3 sets of 2, but has an asymmetry with respect to `BEFORE` and `AFTER`. 
...
|:---------:|:---------:|:---------:|:---------:|
|high | BEFORE | $y\big\rvert{h, B}$ | $\beta_0 + \beta_{lD \cup hB} + f_{lD \cup hB} + b_h + b_{lD \cup hB, h}$ |
|high | DURING | $y\big\rvert{h, D}$ | $\beta_0 + \beta_{lB \cup hD} + f_{lB \cup hD} + b_h$ |
|low  | DURING | $y\big\rvert{l, D}$ | $\beta_0 + \beta_{lD \cup hB} + f_{lD \cup hB}$ |
|low  | BEFORE | $y\big\rvert{l, B}$ | $\beta_0 + \beta_{lB \cup hD} + f_{lB \cup hD}$ |
|high | AFTER  | - | $\beta_0  + f_A + b_h$ |
