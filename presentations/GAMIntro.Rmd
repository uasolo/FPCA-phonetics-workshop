...
```{r}
plot_smooth(m2, view = "time", plot_all = "Category")
```
...
The above is one of **three** ways to specify a smooth interacting with a fixed factor with `mgcv::bam`. A second way is with so-called *binary smooths*, which allow to define smooth contrasts, i.e. they are a way to directly assess for the significance between factor levels with smooths.
...
curves %<>% 
  ...
  group_by(curveId) %>%
  mutate(y = y + trial * h1 * exp(-((time-m1)/s1)**2))
...
summary(m3)
```
The surface plot for the PEAK and NO_PEAK conditions (summed effects) are:
```{r warning=FALSE, message=FALSE}
fvisgam(m3, view = c("time", "trial"), cond = list(Category = "PEAK"), color = c('white', 'blue'))
fvisgam(m3, view = c("time", "trial"), cond = list(Category = "NO_PEAK"), color = c('white', 'blue'))
```
